...
% Page style
\pagestyle{fancy}
\lhead{Dutch Nao Team - Technical Report}
\cfoot{}
\rfoot{\thepage}
...
\begin{figure}[htb]    	
\centering
\fbox{\includegraphics[width=0.7\columnwidth]{TeamPhotoTeheran.jpg}}
...
\begin{figure}[htb]
\centering
\fbox{\includegraphics[width=0.3\columnwidth]{logo_2012.jpg}} % logo may look better when smaller
...
\caption{Hierarchy of files as used in the RoboCup 2012.}
\label{fig:File}
\end{figure}
...
Two widely acknowledged state-of-the-art techniques were used for localisation of both the Nao, and keeping track of the ball. They will be discussed in the following subsections.

\subsubsection{Particle Filter}
The Particle Filter, a sequential Monte Carlo method, is used to find the location of a Nao on the field. The pseudocode is given in algorithm \ref{alg:PF}. 


A typical Particle Filter, as used in the RoboCup, works as follows:
\begin{enumerate}
\item Receive odometry information, which is the walking speed of the Nao.
\item Update all particles.
\item Receive visual information, which consists of positions of found features in the field.
\item Find the samples' new weights, in this step particles outside of the field are given weight 0.
\item Sample using importance sampling (weight-based). 
\item Repeat. 
\end{enumerate}

Only the goal posts were used as features, as line detection was still lacking. Due to the high noise in the measurements of goal posts, the Nao was often unable to locate itself, or particles would converge to undesirable locations (e.g. opposite side of the field, behind the goal). For this reason, the Particle Filter was not used in any of the matches. 

\subsubsection{Kalman Filter}The pseudocode is given in algorithm \ref{alg:KF}.
